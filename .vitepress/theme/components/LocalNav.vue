...
const sectionTitle = computed(() => {
  ...
  for (const item of navItems) {
    ...
    if ('activeMatch' in item && item.activeMatch) {
      try {
        if (new RegExp(item.activeMatch).test(currentPath)) {
          return item.text
        }
      } catch {
        // Ignore invalid custom patterns and continue fallback matching.
      }
    }

    if ('link' in item && item.link && item.link === currentPath) {
      return item.text
    }
  }

  return theme.value.sidebarMenuLabel || 'Menu'
})
</script>
